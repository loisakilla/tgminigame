...
const generateDeck = () => {
  const images = [
    ...
    '/EM2D.jpg',
    '/baltiets.jpg',
    '/EGE2Tv_on_bridge.jpg'
  ];
  const deck = [];
  // Каждое изображение добавляем дважды
  for (let img of images) {
    deck.push({ img, matched: false });
    deck.push({ img, matched: false });
  }
  // Перемешиваем колоду
...
const gameReducer = (state, action) => {
  switch (action.type) {
    case 'FLIP_CARD':
      ...
      if (state.flipped.length < 2 && !state.flipped.includes(action.index) && !state.matched.includes(state.deck[action.index].img)) {
    ...
    case 'CHECK_MATCH':
      ...
      if (state.deck[first].img === state.deck[second].img) {
        const newMatched = [...state.matched, state.deck[first].img];
      ...
      <div className="App">
        ...
        <div className="deck">
          {state.deck.map((card, index) => (
              <div
                  key={index}
                  className={`card ${state.flipped.includes(index) || state.matched.includes(card.img) ? 'flipped show' : ''}`}
                  onClick={() => handleCardClick(index)}
              >
                <img src={card.img} alt="Train" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
              </div>
          ))}
        </div>
        {state.gameOver && (
